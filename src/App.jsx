...
const I = {
  ...
  logo: () => (
    <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
      ...
      />
      <path
        d="M9 10.5C9 8.567 10.567 7 12.5 7H15C17.209 7 19 8.791 19 11C19 13.209 17.209 15 15 15H13C10.791 15 9 16.791 9 19C9 21.209 10.791 23 13 23H15.5C17.433 23 19 21.433 19 19.5"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
      />
...
const SEED_POSTS = [
  {
    id: uid(),
    author: 'Cosmic_Wanderer_4829',
    text: "I still sleep with a stuffed animal and I'm 27. Zero regrets.",
    cat: 'Life',
    hearts: 142,
    liked: false,
    ts: Date.now() - 3600000,
  ...
  {
    id: uid(),
    author: 'Shadow_Echo_7231',
    text: "I pretend to be busy on video calls so people think I'm important. Works every time.",
    cat: 'Work',
    hearts: 89,
    liked: false,
    ts: Date.now() - 7200000,
  ...
  {
    id: uid(),
    author: 'Nebula_Drifter_3341',
    text: "Been learning guitar for 3 years. Still haven't finished a single song.",
    cat: 'Hobby',
    hearts: 204,
    liked: false,
  ...
  {
    id: uid(),
    author: 'Lunar_Specter_9910',
    text: "Traffic alone is my therapy. It's the only silence I get all day.",
    cat: 'Life',
    hearts: 317,
    liked: false,
  ...
  {
    id: uid(),
    author: 'Astral_Signal_2284',
    text: 'Watched The Office 7 times. Cried at the exact same scenes every single time.',
    cat: 'Film',
    hearts: 451,
    liked: false,
  ...
  {
    id: uid(),
    author: 'Quasar_Nomad_5571',
    text: 'I rehearse arguments in the shower that will never happen. I always win them though.',
    cat: 'Life',
    hearts: 388,
    liked: false,
    ts: Date.now() - 320000000,
    comments: 9,
  },
];
...
const SEED_ROOMS = [
  ...
  },
  {
    id: 'seniors',
    name: 'Silver Lounge',
    desc: 'Wisdom & warm conversations',
    members: 380,
    type: 'public',
    active: false,
  },
  {
    id: 'students',
    name: 'Study Hall',
    desc: 'Students helping students',
    members: 634,
    type: 'public',
    active: true,
  },
...
const SEED_MSGS = {
  ...
  latenight: [
    {
      id: uid(),
      from: 'Mystic_Pulse_2209',
      text: '3am thoughts: do fish ever get thirsty?',
      ts: Date.now() - 900000,
    },
  ],
  seniors: [
    {
      id: uid(),
      from: 'Cosmic_Voyager_1188',
      text: 'Good morning! Coffee anyone?',
      ts: Date.now() - 1800000,
    },
  ],
  students: [
    {
      id: uid(),
      from: 'Astral_Observer_4421',
      text: 'Anyone else studying for finals?',
      ts: Date.now() - 1200000,
    },
  ],
  healing: [
    {
      id: uid(),
      from: 'Velvet_Signal_8810',
      text: 'It gets better. Truly.',
      ts: Date.now() - 2400000,
    },
  ],
  random: [
    {
      id: uid(),
      from: 'Shadow_Pilgrim_6634',
      text: 'What is everyone up to tonight?',
      ts: Date.now() - 450000,
    },
  ],
};
  ...
  return `
    ...
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    .card:hover{border-color:${
      T.border2
    } !important;box-shadow:0 2px 16px rgba(0,0,0,${dark ? 0.25 : 0.06});}
    .pb:hover{opacity:0.75;}
    .rr:hover{background:${T.surface2} !important;border-color:${
    T.border2
  } !important;}
...
export default function App() {
  ...
  const toggleHeart = (id) =>
    setPosts((p) =>
      p.map((c) =>
        c.id === id
          ? { ...c, hearts: c.hearts + (c.liked ? -1 : 1), liked: !c.liked }
          : c
      )
    );
  const addPost = (text, cat) => {
    setPosts((p) => [
      {
        id: uid(),
        author: user?.name || guestId,
        text,
        cat,
        hearts: 0,
        liked: false,
  ...
  const sendMsg = (roomId, text) =>
    setMsgs((p) => ({
      ...p,
      [roomId]: [
        ...(p[roomId] || []),
        { id: uid(), from: user?.name || guestId, text, ts: Date.now() },
      ],
    }));
  const signUp = (name) => {
    setUser({ name, id: genId(), joined: Date.now() });
    setModal(null);
  };
  const signIn = (name) => {
    setUser({ name, id: genId(), joined: Date.now() });
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Desktop search — pill style with icon inside */}
        <div
        ...
        {/* Mobile search icon */}
        <button
          className="mobile-search-btn ib"
            ...
            {NAV_ITEMS.map(({ id, label, Icon }) => (
              <button
                ...
                style={{
                  ...
                  borderRadius: 8,
                  border: 'none',
                  background: tab === id ? T.surface2 : 'none',
      ...
      <button
        ...
        style={{
          ...
          justifyContent: 'center',
          color: T.accentText,
          boxShadow: `0 4px 20px rgba(0,0,0,${dark ? 0.5 : 0.18})`,
        }}
      >
        <I.plus />
      </button>
  ...
  return (
    <div>
      ...
      >
        ...
        >
          {(user?.name || guestId).slice(0, 2).toUpperCase()}
        </div>
        <span style={{ color: T.muted, fontSize: '0.85rem', flex: 1 }}>
        ...
        {CATS.map((c) => (
          <button
            ...
            style={{
              ...
              color: cat === c ? T.accentText : T.sub,
              border: `1px solid ${cat === c ? 'transparent' : T.pillBorder}`,
              borderRadius: 99,
              padding: '5px 14px',
              fontSize: '0.73rem',
          ...
          >
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                ...
                <span
                  style={{ fontSize: '0.74rem', fontWeight: 500, color: T.sub }}
                >
                  {p.author}
                </span>
              </div>
      ...
      >
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span
                style={{ fontWeight: 600, fontSize: '0.92rem', color: T.text }}
              >
                {activeRoom.name}
              </span>
            ...
            <div style={{ fontSize: '0.7rem', color: T.sub, marginTop: 1 }}>
              {activeRoom.members.toLocaleString()} members
            </div>
          </div>
        </div>
        <div
            ...
            return (
              ...
              >
                ...
                <div style={{ maxWidth: '70%' }}>
                  ...
                  <div
                    style={{
                      background: isMe ? T.accent : T.surface2,
          ...
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter' && msg.trim()) {
                sendMsg(activeRoom.id, msg.trim());
                setMsg('');
              }
            }}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
        {rooms.map((r) => (
          ...
          >
            ...
            >
              <span style={{ fontSize: '0.72rem', color: T.sub }}>
    ...
    <div>
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            <I.shield /> {user ? 'Member · Anonymous' : 'Guest · Anonymous'}
          </div>
        </div>
        {user && (
          <button
      ...
      >
        {[
          { label: 'Privacy mode', val: 'Anonymous by default' },
          { label: 'Data stored', val: 'Nothing identifiable' },
          { label: 'Network', val: 'Global · Decentralised' },
        ].map((row, i) => (
          <div
            key={row.label}
            style={{
          ...
          >
            ...
            <span
              style={{ fontSize: '0.8rem', fontWeight: 500, color: T.text }}
            >
              {row.val}
            </span>
          </div>
...
function FriendsView({ T }) {
  ...
  const statusColor = (s) =>
    s === 'online' ? T.online : s === 'away' ? T.away : T.muted;
  ...
  return (
    <div>
      ...
      >
        ...
        <span style={{ fontSize: '0.75rem', color: T.sub }}>
        ...
        {filtered.map((f) => (
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: '0.68rem', color: T.sub, marginTop: 1 }}>
                {f.mutual} mutual · {f.status}
              </div>
            </div>
            <button
              style={{
...
function RightStats({ T, posts, rooms }) {
  return (
    ...
    >
      ...
      {[
        { label: 'Confessions', val: posts.length + 1204 },
        { label: 'Active Rooms', val: rooms.filter((r) => r.active).length },
        { label: 'Online Now', val: '2,841' },
      ].map((s, i) => (
        <div
          key={s.label}
          style={{
...
function ConfessModal({ T, onSubmit, onClose, user, guestId }) {
  ...
  return (
    <>
      ...
      >
        <I.shield /> Posting as{' '}
        <b style={{ color: T.text, marginLeft: 2 }}>{user?.name || guestId}</b>
      </div>
      <textarea
        value={text}
        onChange={(e) => setText(e.target.value)}
        placeholder="What's on your mind? No one knows it's you."
        rows={4}
        style={{ ...inp(T), resize: 'none', lineHeight: 1.6, marginBottom: 10 }}
      />
      <div
      ...
      <PBtn
        ...
        onClick={() => text.trim() && onSubmit(text.trim(), cat)}
        disabled={!text.trim()}
      />
    </>
  );
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {['public', 'private'].map((t) => (
              <button
                ...
                style={{
                  flex: 1,
                  background: type === t ? T.accent : T.surface2,
                  border: `1px solid ${type === t ? 'transparent' : T.border}`,
                  borderRadius: 8,
                  padding: '9px 0',
                  fontSize: '0.8rem',
                  fontWeight: type === t ? 600 : 400,
                  color: type === t ? T.accentText : T.sub,
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                  gap: 5,
  ...
  return (
    <>
      ...
      >
        ...
        <div style={{ fontWeight: 600, fontSize: '0.86rem', color: T.text }}>
          {suggested}
        </div>
        <div style={{ fontSize: '0.68rem', color: T.sub, marginTop: 2 }}>
          Auto-generated · Anonymous · No real name needed
        </div>
      </div>
      <div
        style={{
      ...
      >
        <input
          value={name}
          onChange={(e) => setName(e.target.value)}
          placeholder={`Username (default: ${suggested.split('_')[0]})`}
          style={inp(T)}
        />
    ...
    <>
      ...
      >
        <input
          value={name}
          onChange={(e) => setName(e.target.value)}
          placeholder="Your username or Virtual ID"
          style={inp(T)}
        />
        <input
          value={pass}
  ...
  return (
    <>
      ...
      >
        ...
        <div>
          <div style={{ fontWeight: 600, fontSize: '0.88rem', color: T.text }}>
            {user?.name}
          </div>
          <div
            style={{
          ...
          >
            <I.shield /> Virtual Account · Anonymous
          </div>
        </div>
      </div>
      <button
